...
import { DRACULA_FLAVORS } from "./dracula.js";

const EXPECTED_IDS = ["dracula", "dracula-alucard"] as const;

describe("DRACULA_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(DRACULA_FLAVORS.map((flavor) => flavor.id)).toEqual([
      ...EXPECTED_IDS,
    ]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(DRACULA_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of DRACULA_FLAVORS) {
      expect(flavor.family).toBe("dracula");
