...
import { NORD_FLAVORS } from "./nord.js";

const EXPECTED_IDS = ["nord", "nord-light"] as const;

describe("NORD_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(NORD_FLAVORS.map((flavor) => flavor.id)).toEqual([...EXPECTED_IDS]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(NORD_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of NORD_FLAVORS) {
      expect(flavor.family).toBe("nord");
