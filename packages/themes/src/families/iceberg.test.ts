...
import { ICEBERG_FLAVORS } from "./iceberg.js";

const EXPECTED_IDS = ["iceberg-dark", "iceberg-light"] as const;

describe("ICEBERG_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(ICEBERG_FLAVORS.map((flavor) => flavor.id)).toEqual([
      ...EXPECTED_IDS,
    ]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(ICEBERG_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of ICEBERG_FLAVORS) {
      expect(flavor.family).toBe("iceberg");
