...
import { NIGHTFOX_FLAVORS } from "./nightfox.js";

const EXPECTED_IDS = ["nightfox", "dawnfox", "dayfox"] as const;

describe("NIGHTFOX_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(NIGHTFOX_FLAVORS.map((flavor) => flavor.id)).toEqual([
      ...EXPECTED_IDS,
    ]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(NIGHTFOX_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of NIGHTFOX_FLAVORS) {
      expect(flavor.family).toBe("nightfox");
