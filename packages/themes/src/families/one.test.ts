...
import { ONE_FLAVORS } from "./one.js";

const EXPECTED_IDS = ["one-dark", "one-light"] as const;

describe("ONE_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(ONE_FLAVORS.map((flavor) => flavor.id)).toEqual([...EXPECTED_IDS]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(ONE_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of ONE_FLAVORS) {
      expect(flavor.family).toBe("one");
