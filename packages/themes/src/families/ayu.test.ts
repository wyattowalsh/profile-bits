...
import { AYU_FLAVORS } from "./ayu.js";

const EXPECTED_IDS = ["ayu-dark", "ayu-mirage", "ayu-light"] as const;

describe("AYU_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(AYU_FLAVORS.map((flavor) => flavor.id)).toEqual([...EXPECTED_IDS]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(AYU_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of AYU_FLAVORS) {
      expect(flavor.family).toBe("ayu");
