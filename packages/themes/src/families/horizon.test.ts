...
import { HORIZON_FLAVORS } from "./horizon.js";

const EXPECTED_IDS = ["horizon-dark", "horizon-light"] as const;

describe("HORIZON_FLAVORS", () => {
  it("exports the catalog ids", () => {
    expect(HORIZON_FLAVORS.map((flavor) => flavor.id)).toEqual([
      ...EXPECTED_IDS,
    ]);
  });

  it("pairs every flavor with opposite polarity in the same family", () => {
    const byId = new Map(HORIZON_FLAVORS.map((flavor) => [flavor.id, flavor]));
    for (const flavor of HORIZON_FLAVORS) {
      expect(flavor.family).toBe("horizon");
