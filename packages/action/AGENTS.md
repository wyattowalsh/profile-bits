...
- `src/engine.ts` is a fetch/render **port**: no HTTP, no `createHttpClient` / `createWakatimeClient`. Engine enumerates github widgets + coding + json + chips + feed. `main.ts` injects `renderWidget` (join `render.ts` coding/github dispatcher with json/feed compose — do not put rss/http ids in `render.ts`), github-only `probeCapabilities` when github widgets are on, disk `writeFiles`, and existing git/gist ports.
