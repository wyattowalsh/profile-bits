...
- **MODIFIED Action runtime behavior** (compose, not a fourth capability): inject `renderWidget`, github-only `probeCapabilities` when github widgets are on, disk `writeFiles`, and existing git/gist output ports from `runMain`. Keep `engine.ts` HTTP-free.
