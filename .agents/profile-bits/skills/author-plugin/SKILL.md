...
5. Export `{{PLUGIN_ID}}Plugin` + pack-level `{{PLUGIN_ID_UPPER}}_BITS_USED`
   (six-name starter for new packs; union is add-only; update `plugin.test.ts` when unioning)
...
| dest `../` / `../packages/plugins/...` | **Refuse**. Dest is repo-root `packages/plugins/src/<id>/`. |
...
Direct “add MCP” / dest `../packages/plugins/...` → **refuse**.
...
6. Destinations: `packages/plugins/src/<id>/{plugin.ts,plugin.test.ts}`. No per-pack `index.ts`. Templates MUST NOT contain `../`. Refuse dest `../` (including `../packages/plugins/...`). Refuse MCP / `mcp.json`.
7. Never invent flattened `plugin_*_*_*` Action inputs. Read `ActionInputsSchema`.
...
| `title` | Human title. |
| `docsPath` | `"{{DOCS_PATH}}"`. Live packs may differ; read them from disk. Do not hardcode `/generate/<id>/`. |
...
| `bitsUsed` | Pack-level `{{PLUGIN_ID_UPPER}}_BITS_USED` on `{{PLUGIN_ID}}Plugin`. Six-name starter **for new packs only**; author-widget unions imported names (add-only). Complete-existing: if widget-entry arrays exist and pack-level is missing, pack-level = unique union of those arrays. Update `plugin.test.ts` when unioning. |
...
Starter `{{PLUGIN_ID_UPPER}}_BITS_USED` is six layout names **for new
packs only**: `Theme`, `Frame`, `Stack`, `Row`, `Text`, `Muted`. Do **not**
stamp `Stat`, `Bar`, `Chip`, `Avatar`, `Divider` into the pack starter.
author-widget unions names the copied family actually imports (add-only).
Comment: plus those five when the template imports them. Live packs are
subsets of the frozen 11. MDX omits Avatar. Do not copy the six-name
starter over an existing pack.
...
Read live `FIRST_PARTY_INTEGRATION_IDS` and the integrations barrel; do not
assume a closed id set.
...
| Second pack for an id already in `FIRST_PARTY_PLUGIN_IDS` | Refuse duplicate; complete `packages/plugins/src/<id>/` instead |

Worked fill for **new pack using existing integrations**: at least one widget
consumes each named live integration id (may be the same widget).
`plugin.integrations` must equal `deriveIntegrationUnion(...)` and include
every named id. Still OpenSpec+types **before** expanding plugin ids.

Complete-existing pack (id already in `FIRST_PARTY_PLUGIN_IDS`):

- Read live `types.ts` (`WIDGET_INTEGRATIONS`, widget ids, defaults) and
  inventory `packages/plugins/src/<id>/`. Do not hardcode a widget→integration
  map in this skill.
- Do not assume `packages/plugins/src/<id>/` exists until inventory says so.
- If a live pack has widget-entry `bitsUsed` and **no** pack-level `bitsUsed`,
  set pack-level `{{PLUGIN_ID_UPPER}}_BITS_USED` to the unique union of that
  pack’s widget-entry arrays. Widget-entry `bitsUsed` does not close the
  pack-level hole.
...
repo-root, documented here — not inside the template files. Refuse dest
`../` including `../packages/plugins/...`. Dest is repo-root
`packages/plugins/src/<id>/`. Refuse MCP / `mcp.json`.
...
5. **Complete-existing no-clobber.** Inventory live files first. Extend
   rather than overwrite complete sources. Keep any existing widget registry
   ...
   `bitsUsed` (six layout names in the starter **for new packs only**). When
   unioning names later, update `plugin.test.ts` — do not leave a hardcoded
...
- Thin Action inputs only — read `ActionInputsSchema`. Breaking a thin input
  is semver major.
- `plugin_github: true` applies github pack defaults **only** when the
  config file is absent. Do not invent extra `plugin_<id>` pack bools.
...
- [ ] Refused MCP / `mcp.json` and dest `../` (including `../packages/plugins/...`)
- [ ] Complete-existing: inventory; keep any existing widget registry export; pack-level `bitsUsed` = widget-entry union when pack-level is missing (no six-name overwrite); do not shrink live pack-level `bitsUsed`
- [ ] Templates copied with no `../` paths and no per-pack `index.ts`
- [ ] Export `{{PLUGIN_ID}}Plugin` (not `plugin`) with pack-level `bitsUsed`
- [ ] New-pack starter `bitsUsed` is six layout names; do not stamp 11 names into the starter
